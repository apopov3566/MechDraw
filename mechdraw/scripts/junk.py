...
def ikin_b(tip):
    ...
    theta3 = np.arcsin(-(z - l3z) / (l4x - l3x) )
...
def ikin(tip):
    ...
    theta_z_base = np.arctan2(l4z - l3z, l4x - l3x)
    # theta3 = np.arcsin( -(z - l3z) / (l4x - l3x) ) + theta_z_base
    ...
    return (theta1_a, theta2_a, theta3, 0.0)
...
if __name__ == '__main__':
    #joints = np.array([0.63, 0.548, 0., 0.0])
    joints = np.array([0.0, 0.0, 0., 0.])
    # f = np.array([0.98683, -0.08584, 0.19078])
    f = fkin_twist(joints)
    a1, a2, a3, a4 = ikin(f)
    q = ikin(f)
    f_test = fkin_twist(q)
